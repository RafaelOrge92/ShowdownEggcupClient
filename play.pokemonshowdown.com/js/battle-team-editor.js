...
loadSampleSet=function loadSampleSet(setIndex,setName){var _TeamEditorState$samp,_ref5,_ref6,_data$dex$set$species,_data$dex,_data$dex2,_data$stats,_data$stats2;
...
var i=parseInt(target.value);
var editor=_this9.props.editor;
_this9.preserveSetScroll(i,target.closest('.set-button'));
editor.copySet(i);
...
break;
case'move':
_this9.selectMoveResult(name,slot,reverse);
break;
}
editor.save();
_this9.props.onChange==null||_this9.props.onChange();
...
case 13:
case 9:
var _value3=editor.selectSearchValue();
if(((_editor$innerFocus=editor.innerFocus)==null?void 0:_editor$innerFocus.type)!=='move')searchBox.value=_value3||'';
if(_value3!==null){var _editor$innerFocus2;
...
var _value3$split=_value3.split('|'),name=_value3$split[0],moveSlot=_value3$split[1];
...
};return _this9;}_inheritsLoose(TeamWizard,_preact$Component3);var _proto4=TeamWizard.prototype;_proto4.startFocusAnimation=function startFocusAnimation(source){if(this.props.editor.innerFocus)return;var setButton=source==null?void 0:source.closest('.set-button');if(!setButton)return;var rect=setButton.getBoundingClientRect();this.focusAnimationStartLocation={rect:{left:rect.left,top:rect.top}};};_proto4.finishFocusAnimation=function finishFocusAnimation(){var _window$PS5;var start=this.focusAnimationStartLocation;if(!start)return;this.focusAnimationStartLocation=null;if((_window$PS5=window.PS)!=null&&_window$PS5.prefs.noanim||window.matchMedia!=null&&window.matchMedia('(prefers-reduced-motion: reduce)').matches)return;var setButton=this.base.querySelector('.team-focus-editor .set-button');if(!setButton)return;var rect=setButton.getBoundingClientRect();var dx=start.rect.left-rect.left;var dy=start.rect.top-rect.top;if(!dx&&!dy)return;setButton.animate==null||setButton.animate([{transform:"translate("+dx+"px, "+dy+"px)"},{transform:'translate(0, 0)'}],{duration:250,easing:'cubic-bezier(.2, 0, .2, 1)'});};_proto4.preserveSetScroll=function preserveSetScroll(index,elem){if(index===undefined||elem===null)return;this.pendingSetScrollRestore={index:index,top:elem.getBoundingClientRect().top};};_proto4.restorePendingSetScroll=function restorePendingSetScroll(){if(!this.base)return;var restore=this.pendingSetScrollRestore;if(!restore)return;this.pendingSetScrollRestore=null;var setButton=this.base.querySelector(".set-button[data-set-index=\""+restore.index+"\"]");if(!setButton)return;var dy=setButton.getBoundingClientRect().top-restore.top;if(!dy)return;var scrollParent=this.getSetScrollParent(setButton);if(scrollParent){scrollParent.scrollTop+=dy;}else{window.scrollBy(0,dy);}};_proto4.getSetScrollParent=function getSetScrollParent(elem){for(var parent=elem.parentElement;parent;parent=parent.parentElement){var style=getComputedStyle(parent);if(!/(auto|scroll)/.test(style.overflowY))continue;if(parent.scrollHeight<=parent.clientHeight)continue;return parent;}return null;};_proto4.changeFocus=function changeFocus(focus){var editor=this.props.editor;editor.innerFocus=focus;if(!focus){this.props.onUpdate();return;}var set=editor.sets[focus.setIndex];if(focus.type==='details'){this.setSearchBox=set.name||'';}else if(focus.type!=='stats'&&focus.type!=='import'){var _value4;if(focus.type==='pokemon')_value4=(set==null?void 0:set.species)||'';else if(focus.type==='item')_value4=set.item;else if(focus.type==='ability')_value4=set.ability;editor.setSearchType(focus.type,focus.setIndex,_value4,focus.typeIndex);this.resetScroll();this.setSearchBox=_value4||'';}this.props.onUpdate();};_proto4.renderSet=function renderSet(set,i){var _TeamEditorState$clip6;var editor=this.props.editor;var sprite=Dex.getTeambuilderSprite(set,editor.dex);var spriteClass=set&&Dex.getTeambuilderSpriteData(set,editor.dex).pixelated?' pixelated':'';if(!set){return preact.h("div",{"class":"set-button set-wizard","data-set-index":i},preact.h("div",{style:"text-align:right"},editor.deletedSet?preact.h("button",{onClick:this.undeleteSet,"class":"option"},preact.h("i",{"class":"fa fa-undo","aria-hidden":true})," Undo delete"):preact.h("button",{"class":"option",style:"visibility:hidden"},preact.h("i",{"class":"fa fa-trash","aria-hidden":true})," Delete")),preact.h("table",null,preact.h("tr",null,preact.h("td",{rowSpan:2,"class":"set-pokemon"},preact.h("div",{"class":"border-collapse"},preact.h("button",{"class":"button button-first cur",onClick:this.setFocus,value:"set-"+i+"-pokemon"},preact.h("span",{"class":"sprite"+spriteClass,style:sprite},preact.h("span",{"class":"sprite-inner"},preact.h("strong",{"class":"label"},"Pokemon")," ",preact.h("em",null,"(choose species)")))))),preact.h("td",{colSpan:2,"class":"set-details"}),preact.h("td",{rowSpan:2,"class":"set-moves"}),preact.h("td",{rowSpan:2,"class":"set-stats"})),preact.h("tr",null,preact.h("td",{"class":"set-ability"}),preact.h("td",{"class":"set-item"}))));}while(set.moves.length<4)set.moves.push('');var overfull=set.moves.length>4?' overfull':'';var cur=function(t){var _editor$innerFocus4;return editor.readonly||((_editor$innerFocus4=editor.innerFocus)==null?void 0:_editor$innerFocus4.type)===t&&editor.innerFocus.setIndex===i?' cur':'';};var species=editor.dex.species.get(set.species);var isCur=(_TeamEditorState$clip6=TeamEditorState.clipboard)!=null&&(_TeamEditorState$clip6=_TeamEditorState$clip6.teams)!=null&&(_TeamEditorState$clip6=_TeamEditorState$clip6[editor.team.key])!=null&&_TeamEditorState$clip6.sets[i]?' cur':'';return preact.h("div",{"class":"set-button set-wizard"+isCur,"data-set-index":i},preact.h("div",{style:"text-align:right"},preact.h("button",{"class":"option",onClick:this.copySet,value:i},preact.h("i",{"class":"fa fa-copy","aria-hidden":true})," ",isCur?"Deselect":TeamEditorState.clipboard?"Add to clipboard":editor.readonly?"Copy":"Copy/Move")," ",!(TeamEditorState.clipboard||editor.readonly)&&preact.h("button",{"class":"option",onClick:this.deleteSet,value:i},preact.h("i",{"class":"fa fa-trash","aria-hidden":true})," Delete")),preact.h("table",null,preact.h("tr",null,preact.h("td",{rowSpan:2,"class":"set-pokemon"},preact.h("div",{"class":"border-collapse"},preact.h("button",{"class":"button button-first"+cur('pokemon'),onClick:this.setFocus,value:"set-"+i+"-pokemon"},preact.h("span",{"class":"sprite"+spriteClass,style:sprite},preact.h("span",{"class":"sprite-inner"},preact.h("strong",{"class":"label"},"Pokemon")," ",set.species))))),preact.h("td",{colSpan:2,"class":"set-details"},preact.h("div",{"class":"border-collapse"},preact.h("button",{"class":"button button-middle"+cur('details'),onClick:this.setFocus,value:"set-"+i+"-details"},preact.h("span",{"class":"detailcell"},preact.h("strong",{"class":"label"},"Types")," ",species.types.map(function(type){return preact.h("div",null,preact.h(PSIcon,{type:type}));})),preact.h("span",{"class":"detailcell"},preact.h("strong",{"class":"label"},"Level")," ",set.level||editor.defaultLevel,editor.narrow&&set.shiny&&preact.h(preact.Fragment,null,preact.h("br",null),preact.h("img",{src:Dex.resourcePrefix+"sprites/misc/shiny.png",width:22,height:22,alt:"Shiny"})),!editor.narrow&&set.gender&&set.gender!=='N'&&preact.h(preact.Fragment,null,preact.h("br",null),preact.h("img",{src:Dex.fxPrefix+"gender-"+set.gender.toLowerCase()+".png",alt:set.gender,width:"7",height:"10","class":"pixelated"}))),!!(!editor.narrow&&(set.shiny||editor.gen>=2))&&preact.h("span",{"class":"detailcell"},preact.h("strong",{"class":"label"},"Shiny")," ",set.shiny?preact.h("img",{src:Dex.resourcePrefix+"sprites/misc/shiny.png",width:22,height:22,alt:"Yes"}):"\u2014"),editor.gen===9&&!editor.isChampions&&preact.h("span",{"class":"detailcell"},preact.h("strong",{"class":"label"},"Tera")," ",preact.h(PSIcon,{type:set.teraType||species.requiredTeraType||species.types[0]})),editor.hpTypeMatters(set)&&preact.h("span",{"class":"detailcell"},preact.h("strong",{"class":"label"},"H.P.")," ",preact.h(PSIcon,{type:editor.getHPType(set)}))))),preact.h("td",{rowSpan:2,"class":"set-moves"},preact.h("div",{"class":"border-collapse"},preact.h("button",{"class":"button button-middle"+cur('move')+overfull,onClick:this.setFocus,value:"set-"+i+"-move"},preact.h("strong",{"class":"label"},"Moves")," ",set.moves.map(function(move,mi){return preact.h("div",null,!editor.narrow&&preact.h("small",{"class":"gray"},"\u2022"),mi>=4?preact.h("span",{"class":"message-error"},move||editor.narrow&&'-'||''):move||editor.narrow&&'-');}),!set.moves.length&&preact.h("em",null,"(no moves)")))),preact.h("td",{rowSpan:2,"class":"set-stats"},preact.h("div",{"class":"border-collapse"},preact.h("button",{"class":"button button-last"+cur('stats'),onClick:this.setFocus,value:"set-"+i+"-stats"},StatForm.renderStatGraph(set,this.props.editor,true))))),preact.h("tr",null,preact.h("td",{"class":"set-ability"},preact.h("div",{"class":"border-collapse"},preact.h("button",{"class":"button button-middle"+cur('ability'),onClick:this.setFocus,value:"set-"+i+"-ability"},(editor.gen>=3||set.ability)&&preact.h(preact.Fragment,null,preact.h("strong",{"class":"label"},"Ability")," ",set.ability!=='No Ability'&&set.ability||(!set.ability?preact.h("em",null,"(choose ability)"):preact.h("em",null,"(no ability)")))))),preact.h("td",{"class":"set-item"},preact.h("div",{"class":"border-collapse"},preact.h("button",{"class":"button button-middle"+cur('item'),onClick:this.setFocus,value:"set-"+i+"-item"},(editor.gen>=2||set.item)&&preact.h(preact.Fragment,null,set.item&&preact.h(PSIcon,{item:set.item}),preact.h("strong",{"class":"label"},"Item")," ",set.item||preact.h("em",null,"(no item)"))))))),preact.h("button",{"class":"button set-nickname"+cur('details'),onClick:this.setFocus,value:"set-"+i+"-details"},preact.h("strong",{"class":"label"},"Nickname")," ",editor.getNickname(set)));};_proto4.clearSearchBox=function clearSearchBox(){var searchBox=this.base.querySelector('input[name=value]');if(searchBox){searchBox.value='';if(!TeamEditor.probablyMobile())searchBox.focus();}};_proto4.selectMoveResult=function selectMoveResult(name,slot,reverse){var _editor$sets2;var editor=this.props.editor;var setIndex=editor.innerFocus.setIndex;var set=(_editor$sets2=editor.sets)[setIndex]||(_editor$sets2[setIndex]={species:'',moves:[]});if(slot){var i=parseInt(slot)-1;if(set.moves[i]){set.moves[i]='';if(i===set.moves.length-1){while(set.moves.length>4&&!set.moves[set.moves.length-1]){set.moves.pop();}}if(set.moves.length>4&&i<set.moves.length-1){set.moves[i]=set.moves.pop();}}}else if(set.moves.includes(name)){set.moves.splice(set.moves.indexOf(name),1);}else{for(var _i40=0;_i40<set.moves.length+1;_i40++){if(!set.moves[_i40]){set.moves[_i40]=name;break;}}}if(set.moves.length===4&&set.moves.every(Boolean)&&!this.isForm){this.changeFocus({setIndex:setIndex,type:reverse?'item':'stats',typeIndex:-1});}else if(reverse){this.changeFocus({setIndex:setIndex,type:'item',typeIndex:-1});}else{if(editor.search.query){this.resetScroll();}editor.updateSearchMoves(set);}};_proto4.
...
var isSearchMode=type!=='stats'&&type!=='details'&&type!=='import';
...
return preact.h("div",{"class":"team-focus-editor",onKeyDown:editor.handleParentKeyDown},
preact.h("div",{"class":isSearchMode&&((set==null?void 0:set.moves.length)||0)>5?'team-focus-top':''},
preact.h("ul",{"class":"tabbar"},
preact.h("li",{"class":"home-li"},preact.h("button",{"class":"button",onClick:this.closeInnerFocus},
...
"class":"button picontab"+cur(i),onClick:_this10.setFocus,
value:"set-"+i+"-"+type},
...
isSearchMode&&preact.h("div",{"class":"searchboxwrapper pad",onClick:this.handleClickFilters},
preact.h("input",{
...
search:editor.search,hideFilters:true,
onSelect:this.selectResult},

type==='ability'&&preact.h(SetSourceButtons,{
...
return preact.h("div",{"class":"teameditor"+(editor.readonly?' readonly':'')},
editor.sets.map(function(set,i){return[
pasteControls(i),
_this11.renderSet(set,i)];}
),
pasteControls(editor.sets.length),
editor.canAdd()&&preact.h("p",null,preact.h("button",{
...
TeamEditorForm=function(_TeamWizard2){function TeamEditorForm(){var _this12;for(var _len4=arguments.length,args=new Array(_len4),_key6=0;_key6<_len4;_key6++){args[_key6]=arguments[_key6];}_this12=_TeamWizard2.call.apply(_TeamWizard2,[this].concat(args))||this;_this12.
isForm=true;_this12.

pendingFocus=null;_this12.
pendingFocusValue=null;_this12.
pendingFocusSelection=null;_this12.

pendingFocusButton=false;_this12.
mouseDownTextbox=null;_this12.
...
var target=ev.currentTarget;
if(_this12.mouseDownTextbox===target)return;
_this12.openFocusTextbox(target);
};_this12.
mouseDownField=function(ev){
if(ev.button!==0)return;
var target=ev.currentTarget;
if(document.activeElement===target)return;
_this12.mouseDownTextbox=target;
document.addEventListener('mouseup',_this12.mouseUpField,{once:true});
target.focus();
target.select();
ev.preventDefault();
};_this12.
mouseUpField=function(ev){
var target=_this12.mouseDownTextbox;
_this12.mouseDownTextbox=null;
...
if(type==='nickname'){
_this12.commitField(target);
return;
}
var focus=_this12.props.editor.innerFocus;
if(!focus){
_this12.openFocusTextbox(target);
focus=_this12.pendingFocus;
}
if(((_focus=focus)==null?void 0:_focus.type)==='move'&&focus.typeIndex>=0&&!target.value){

_this12.props.editor.search.prependResults=[['move',"_"+(focus.typeIndex+1)+"_"]];
_this12.props.editor.search.results=null;
}else if(((_focus2=focus)==null?void 0:_focus2.type)==='item'&&!target.value){

_this12.props.editor.search.prependResults=[['item','']];
_this12.props.editor.search.results=null;
}
_this12.props.editor.setSearchValue(target.value);
_this12.resetScroll();
_this12.forceUpdate();
};_this12.
blurField=function(ev){
_this12.commitField(ev.currentTarget);
};_this12.
...
if(!_this12.props.editor.innerFocus&&ev.keyCode===9){
var target=ev.currentTarget;
var focus=_this12.props.editor.parseFocus(target.getAttribute('data-focus'));
if(!_this12.commitField(target)){
ev.preventDefault();
return;
}
if(_this12.focusAdjacentField(focus,ev.shiftKey)){
ev.preventDefault();
}
return;
}
_this12.keyDownSearchInput(ev,false);
};_this12.
...
_this12.keyDownSearchInput(ev,true);
};_this12.
clickPanelButton=function(ev){
var editor=_this12.props.editor;
if(editor.readonly)return;
var target=ev.currentTarget;
var focus=editor.parseFocus(target.value);
if(editor.stringifyFocus(editor.innerFocus)===target.value){
_this12.pendingFocus=focus;
_this12.pendingFocusButton=false;
_this12.forceUpdate();
return;
}
_this12.startFocusAnimation(target);
_this12.changeFocus(focus,false);
};_this12.
keyDownPanelButton=function(ev){
if(ev.keyCode!==9)return;
var target=ev.currentTarget;
var focus=_this12.props.editor.parseFocus(target.value);
if(!focus)return;
_this12.focusAdjacentField(focus,ev.shiftKey);
ev.preventDefault();
};_this12.
closeInnerFocus=function(ev){var _querySelector$getBou,_querySelector,_document$activeEleme;
var focus=_this12.props.editor.innerFocus;
if(!focus)return;
var expectedTop=(_querySelector$getBou=(_querySelector=_this12.base.querySelector(
...
_this12.props.editor.innerFocus=null;
_this12.pendingFocus=null;
_this12.forceUpdate(function(){
var target=_this12.getOuterFocusTarget(focus,restoreNickname);
var setButton=_this12.getOuterSetButton(focus);
if(target&&(target.name==='nickname'||!target.classList.contains('set-field'))){
target.focus({preventScroll:true});
target.select==null||target.select();
}
_this12.restoreOuterSetScroll(setButton||target||null,expectedTop);
});
_this12.props.onUpdate();
ev==null||ev.stopImmediatePropagation();
ev==null||ev.preventDefault();
};_this12.
handleKeyDown=function(ev){
if(ev.keyCode!==27)return;
_this12.closeInnerFocus(ev);
return false;
};_this12.
...
selectResult=function(type,name,slot,reverse){var _this12$props$editor$;
if(type===null){
_this12.resetScroll();
_this12.forceUpdate();
}else if(!type){
var searchBox=_this12.base.querySelector('input[name=value]');
if(searchBox){
searchBox.value='';
searchBox.focus();
}
_this12.props.editor.setSearchValue('');
_this12.resetScroll();
_this12.forceUpdate();
}else if(type==='move'&&((_this12$props$editor$=_this12.props.editor.innerFocus)==null?void 0:_this12$props$editor$.typeIndex)===-1){
_this12.setMoveResult(name,slot,reverse);
}else{
_this12.setFocusedValue(name);
}
};return _this12;}_inheritsLoose(TeamEditorForm,_TeamWizard2);var _proto5=TeamEditorForm.prototype;_proto5.openFocusTextbox=function openFocusTextbox(target){var editor=this.props.editor;if(editor.readonly)return;var focus=editor.parseFocus(target.getAttribute('data-focus'));if(!focus)return;if(this.pendingFocus)return;this.pendingFocusValue=target.value;this.pendingFocusSelection=[target.selectionStart,target.selectionEnd,target.selectionDirection||undefined];target.classList.remove('incomplete');this.startFocusAnimation(target);this.changeFocus(focus);};_proto5.changeFocus=function changeFocus(focus){var focusButton=arguments.length>1&&arguments[1]!==undefined?arguments[1]:false;var editor=this.props.editor;editor.innerFocus=focus;this.pendingFocus=focus;this.pendingFocusButton=focusButton;if(!focus){this.props.onUpdate();return;}var set=editor.sets[focus.setIndex];if(focus.type!=='details'&&focus.type!=='stats'&&focus.type!=='import'){var _set$moves7;var _value5='';if(focus.type==='pokemon')_value5=(set==null?void 0:set.species)||'';else if(focus.type==='item')_value5=(set==null?void 0:set.item)||'';else if(focus.type==='ability')_value5=(set==null?void 0:set.ability)||'';else if(focus.type==='move'&&focus.typeIndex>=0)_value5=(set==null||(_set$moves7=set.moves)==null?void 0:_set$moves7[focus.typeIndex])||'';editor.setSearchType(focus.type,focus.setIndex,_value5,focus.typeIndex);this.resetScroll();}this.props.onUpdate();};_proto5.componentDidMount=function componentDidMount(){this.props.editor.handleParentKeyDown=this.handleKeyDown;};_proto5.componentWillUnmount=function componentWillUnmount(){this.props.editor.handleParentKeyDown=undefined;};_proto5.componentDidUpdate=function componentDidUpdate(){this.finishFocusAnimation();var editor=this.props.editor;var focus=this.pendingFocus;if(focus){var focusValue=editor.stringifyFocus(focus);var input=this.base.querySelector((focus.type==='details'||focus.type==='stats'||focus.type==='import')&&this.pendingFocusButton?"button[name=\""+focus.type+"\"][value=\""+focusValue+"\"]":focus.type==='details'?"div[aria-label=Details] input:not([name=nickname]), div[aria-label=Details] select":focus.type==='stats'?"div[aria-label=Stats] input":focus.type==='import'?"div[aria-label=\"Import/Export\"] textarea":focus.type==='move'&&focus.typeIndex===-1?"input[name=value]":"input.set-field[data-focus=\""+focusValue+"\"]");if(input){if(focus.type!=='details'&&focus.type!=='stats'&&focus.type!=='import'&&!(focus.type==='move'&&focus.typeIndex===-1)){var _this$pendingFocusVal;input.value=(_this$pendingFocusVal=this.pendingFocusValue)!=null?_this$pendingFocusVal:this.getInputValue(focus.setIndex,focus.type,focus.typeIndex);input.classList.remove('incomplete');}input.focus();if(this.pendingFocusSelection&&input instanceof HTMLInputElement){input.setSelectionRange==null||input.setSelectionRange.apply(input,this.pendingFocusSelection);}else{input.select==null||input.select();}this.pendingFocus=null;this.pendingFocusValue=null;this.pendingFocusSelection=null;}}var activeElement=document.activeElement;for(var _i42=0,_querySelectorAll2=this.base.querySelectorAll('input.set-field');_i42<_querySelectorAll2.length;_i42++){var _input=_querySelectorAll2[_i42];if(_input===activeElement)continue;var curFocus=editor.parseFocus(_input.getAttribute('data-focus'));_input.value=this.getInputValue(curFocus.setIndex,curFocus.type,curFocus.typeIndex);}var searchBox=this.base.querySelector('input[name=value]');var filters=this.base.querySelector('.dexlist-filters');if(searchBox){if(filters){var _filters$getBoundingC2=filters.getBoundingClientRect(),width=_filters$getBoundingC2.width;searchBox.style.paddingLeft=width+5+"px";}else{searchBox.style.paddingLeft="3px";}}this.restorePendingSetScroll();};_proto5.getInputValue=function getInputValue(setIndex,type,typeIndex){var set=this.props.editor.sets[setIndex];if(!set)return'';switch(type){case'pokemon':return set.species||'';case'item':return set.item||'';case'ability':return set.ability||'';case'move':return set.moves[typeIndex]||'';case'nickname':return set.name||'';default:return'';}};_proto5.canonicalValue=function canonicalValue(type,value){var editor=this.props.editor;if(!value.trim())return'';switch(type){case'pokemon':{var species=editor.dex.species.get(value);return species.exists?species.name:null;}case'item':{if(toID(value)==='noitem')return'';var item=editor.dex.items.get(value);return item.exists?item.name:null;}case'ability':{if(toID(value)==='noability')return'';var ability=editor.dex.abilities.get(value);return ability.exists?ability.name:null;}case'move':{var move=editor.dex.moves.get(value);return move.exists?move.name:null;}default:return value;}};_proto5.commitField=function commitField(target,selectNext,reverse){var _this$props$onChange3,_this$props4;var editor=this.props.editor;var focus=editor.parseFocus(target.getAttribute('data-focus'));if(!focus)return true;if(focus.type==='nickname'){var _this$props$onChange2,_this$props3;var set=editor.sets[focus.setIndex];if(!set)return true;var name=target.value.trim();if(name){set.name=name;}else{delete set.name;}editor.save();(_this$props$onChange2=(_this$props3=this.props).onChange)==null||_this$props$onChange2.call(_this$props3);this.forceUpdate();return true;}var canonical=this.canonicalValue(focus.type,target.value);if(canonical===null){target.classList.add('incomplete');canonical=target.value;}else{target.classList.remove('incomplete');}if(focus.type==='pokemon'){var _editor$sets3,_focus$setIndex2;if(!canonical)return true;var _set2=(_editor$sets3=editor.sets)[_focus$setIndex2=focus.setIndex]||(_editor$sets3[_focus$setIndex2]={species:'',moves:[]});editor.changeSpecies(_set2,canonical);target.value=_set2.species;}else{var _set3=editor.sets[focus.setIndex];if(!_set3)return true;switch(focus.type){case'item':if(canonical)_set3.item=canonical;else delete _set3.item;target.value=canonical;break;case'ability':if(canonical)_set3.ability=canonical;else delete _set3.ability;target.value=canonical;break;case'move':if(focus.typeIndex>=_set3.moves.length&&!canonical)return true;while(_set3.moves.length<=focus.typeIndex)_set3.moves.push('');_set3.moves[focus.typeIndex]=canonical;target.value=canonical;break;}}editor.save();(_this$props$onChange3=(_this$props4=this.props).onChange)==null||_this$props$onChange3.call(_this$props4);this.forceUpdate();if(selectNext)this.focusAdjacentField(focus,!!reverse);return true;};_proto5.getFocusedSetField=function getFocusedSetField(){var editor=this.props.editor;var focus=editor.innerFocus;if(!focus||focus.type==='details'||focus.type==='stats'||focus.type==='import'){return null;}if(focus.type==='move'&&focus.typeIndex===-1)return null;return this.base.querySelector("input.set-field[data-focus=\""+editor.stringifyFocus(focus)+"\"]");};_proto5.focusFocusedSetField=function focusFocusedSetField(){var input=this.getFocusedSetField();if(!input)return false;var focus=this.props.editor.parseFocus(input.getAttribute('data-focus'));if(!focus)return false;input.value=this.getInputValue(focus.setIndex,focus.type,focus.typeIndex);input.classList.remove('incomplete');input.focus();input.select();return true;};_proto5.clearSearchFilters=function clearSearchFilters(){while(true){if(!this.props.editor.search.removeFilter())return;}};_proto5.keyDownSearchInput=function keyDownSearchInput(ev,inSearchBox){var editor=this.props.editor;var input=ev.currentTarget;switch(ev.keyCode){case 8:if(input.selectionStart===0&&input.selectionEnd===0){if(!editor.search.removeFilter()&&inSearchBox&&!input.value){if(this.focusFocusedSetField())ev.preventDefault();break;}editor.setSearchValue(input.value);this.resetScroll();this.forceUpdate();}break;case 27:if(inSearchBox){input.value='';this.clearSearchFilters();editor.setSearchValue('');this.resetScroll();if(!this.focusFocusedSetField())break;this.forceUpdate();ev.preventDefault();ev.stopImmediatePropagation();}break;case 38:editor.search.moveResultIndex(-1);ev.preventDefault();break;case 40:editor.search.moveResultIndex(1);ev.preventDefault();break;case 37:case 39:ev.stopImmediatePropagation();break;case 13:case 9:{if(ev.keyCode===9&&ev.shiftKey){this.commitField(input,true,true);this.tryDeleteEmptyMoveSlot(input);ev.preventDefault();return;}var _value6=editor.selectSearchValue();if(_value6===''&&input.value){this.commitField(input,true);}else if(_value6!==null){var _editor$innerFocus6;var _value6$split=_value6.split('|'),name=_value6$split[0],moveSlot=_value6$split[1];if(((_editor$innerFocus6=editor.innerFocus)==null?void 0:_editor$innerFocus6.type)==='move'&&editor.innerFocus.typeIndex===-1){this.setMoveResult(name,moveSlot);}else{this.setFocusedValue(name,ev.shiftKey);}this.tryDeleteEmptyMoveSlot(input);if(inSearchBox)input.value='';}else{if(inSearchBox){input.value='';}else{var focus=editor.parseFocus(input.getAttribute('data-focus'));if(focus)input.value=this.getInputValue(focus.setIndex,focus.type,focus.typeIndex);input.classList.remove('incomplete');var searchBox=this.base.querySelector('input[name=value]');if(searchBox){searchBox.value='';searchBox.focus();}}editor.setSearchValue('');this.resetScroll();this.forceUpdate();}ev.preventDefault();break;}}};_proto5.tryDeleteEmptyMoveSlot=function tryDeleteEmptyMoveSlot(input){var _editor$sets$focus$se,_editor$innerFocus7;if(input.value)return false;var editor=this.props.editor;var focus=editor.parseFocus(input.getAttribute('data-focus'));if((focus==null?void 0:focus.type)!=='move'||focus.typeIndex<0)return false;var moves=(_editor$sets$focus$se=editor.sets[focus.setIndex])==null?void 0:_editor$sets$focus$se.moves;if(!moves)return false;if(moves[focus.typeIndex])return false;moves.splice(focus.typeIndex,1);if((editor==null||(_editor$innerFocus7=editor.innerFocus)==null?void 0:_editor$innerFocus7.type)==='move'&&editor.innerFocus.typeIndex>focus.typeIndex){editor.innerFocus.typeIndex--;}input.value=moves[focus.typeIndex]||'';this.forceUpdate();return true;};_proto5.restoreOuterSetScroll=function restoreOuterSetScroll(target,expectedTop){if(!target||expectedTop===null)return;var setButton=target.closest('.set-button')||target;var dy=setButton.getBoundingClientRect().top-expectedTop;if(!dy)return;var scrollParent=this.getScrollParent(setButton);if(scrollParent){scrollParent.scrollTop+=dy;}else{window.scrollBy(0,dy);}};_proto5.getScrollParent=function getScrollParent(elem){for(var parent=elem.parentElement;parent;parent=parent.parentElement){var style=getComputedStyle(parent);if(!/(auto|scroll)/.test(style.overflowY))continue;if(parent.scrollHeight<=parent.clientHeight)continue;return parent;}return null;};_proto5.getOuterFocusTarget=function getOuterFocusTarget(focus){var restoreNickname=arguments.length>1&&arguments[1]!==undefined?arguments[1]:false;if(restoreNickname){return this.base.querySelector("input[data-focus=\"set-"+focus.setIndex+"-nickname\"]");}if(focus.type==='details'||focus.type==='stats'||focus.type==='import'){return this.base.querySelector("button[name=\""+focus.type+"\"][value=\""+this.props.editor.stringifyFocus(focus)+"\"]");}if(focus.type==='move'&&focus.typeIndex===-1){return this.base.querySelector("button[value=\""+this.props.editor.stringifyFocus(focus)+"\"]");}};_proto5.getOuterSetButton=function getOuterSetButton(focus){if(focus.setIndex>=this.props.editor.sets.length)return null;return this.base.querySelectorAll('.teameditor > .set-button.set-form')[focus.setIndex]||null;};_proto5.removeDuplicateMove=function removeDuplicateMove(name){var _this$props$onChange4,_this$props5;var editor=this.props.editor;var focus=editor.innerFocus;if(!name)return false;if((focus==null?void 0:focus.type)!=='move')return false;var set=editor.sets[focus.setIndex];if(!set)return false;var moveIndex=set.moves.indexOf(name);if(moveIndex<0||moveIndex===focus.typeIndex)return false;set.moves.splice(moveIndex,1);var emptyIndex=0;while(emptyIndex<4&&set.moves[emptyIndex])emptyIndex++;if(emptyIndex>=4)emptyIndex=focus.typeIndex;this.changeFocus({setIndex:focus.setIndex,type:'move',typeIndex:emptyIndex});editor.save();(_this$props$onChange4=(_this$props5=this.props).onChange)==null||_this$props$onChange4.call(_this$props5);this.forceUpdate();return true;};_proto5.setFocusedValue=function setFocusedValue(name,reverse){var focus=this.props.editor.innerFocus;if(!focus)return;if(this.removeDuplicateMove(name))return;var input=this.base.querySelector("input.set-field[data-focus=\""+this.props.editor.stringifyFocus(focus)+"\"]");if(!input)return;input.value=name;this.commitField(input,true,reverse);};_proto5.setMoveResult=function setMoveResult(name,slot,reverse){var _this$props$onChange5,_this$props6;this.selectMoveResult(name,slot,reverse);this.props.editor.save();(_this$props$onChange5=(_this$props6=this.props).onChange)==null||_this$props$onChange5.call(_this$props6);this.forceUpdate();};_proto5.
...
var prevButton=this.base.querySelector(".team-focus-editor .set-button.set-form button[name=delete][value=\""+
focus.setIndex+"\"]"
)||this.base.querySelector(".team-focus-editor .tabbar button[name=addpokemon]"

)||this.base.querySelector(".teameditor > .set-button.set-form button[name=delete][value=\""+
focus.setIndex+"\"]"
...
showItem=function showItem(set){
var editor=this.props.editor;
return!!(editor.gen>1&&!editor.isLetsGo||set.item);
};_proto5.
showAbility=function showAbility(set){
var editor=this.props.editor;
...
renderSet=function renderSet(set,i){var _TeamEditorState$clip7,_this13=this;
var editor=this.props.editor;
...
if(!set){
return preact.h("div",{"class":"set-button set-form","data-set-index":i},
...
var isCur=(_TeamEditorState$clip7=TeamEditorState.clipboard)!=null&&(_TeamEditorState$clip7=_TeamEditorState$clip7.teams)!=null&&(_TeamEditorState$clip7=_TeamEditorState$clip7[editor.team.key])!=null&&_TeamEditorState$clip7.sets[i]?' cur':'';
var overfull=set.moves.length>5?' overfull':set.moves.length>4?' overfull overfull5':'';
return preact.h("div",{"class":"set-button set-form"+isCur,"data-set-index":i},
preact.h("div",{style:"text-align:right"},
...
preact.h("div",{"class":"moverow"},_this13.renderInput(i,'move',move,moveIndex)));}
)
)),
preact.h("td",{rowSpan:2,"class":"set-stats"},
...
this.showAbility(set)&&preact.h("label",{"class":"label"},"Ability ",
...
this.showItem(set)&&preact.h(preact.Fragment,null,
...
};return TeamEditorForm;}(TeamWizard);
...
SetImportForm=function(_preact$Component4){function SetImportForm(){var _this14;for(var _len5=arguments.length,args=new Array(_len5),_key7=0;_key7<_len5;_key7++){args[_key7]=arguments[_key7];}_this14=_preact$Component4.call.apply(_preact$Component4,[this].concat(args))||this;_this14.
...
var editor=_this14.props.editor;
if(editor.readonly||!_this14.textbox)return;
_this14.textbox.value=_this14.revertText;
if(!_this14.revertSetToRevertPoint())return;
_this14.textbox.focus();
_this14.textbox.select();
_this14.setState({error:'',copied:false,dirty:false});
};_this14.
copyText=function(){
if(!_this14.textbox)return;
_this14.textbox.select();
document.execCommand('copy');
_this14.setState({copied:true});
};_this14.
loadSampleSet=function(setName){
var _this14$props=_this14.props,editor=_this14$props.editor,setIndex=_this14$props.setIndex;
if(!editor.loadSampleSet(setIndex,setName))return;
_this14.refreshText();
_this14.props.onChange();
};_this14.
loadUserSet=function(setName){
var _this14$props2=_this14.props,editor=_this14$props2.editor,setIndex=_this14$props2.setIndex;
if(!editor.loadUserSet(setIndex,setName))return;
_this14.refreshText();
_this14.props.onChange();
};_this14.
inputText=function(){
var _this14$props3=_this14.props,editor=_this14$props3.editor,setIndex=_this14$props3.setIndex;
if(editor.readonly||!_this14.textbox)return;
var dirty=_this14.textbox.value!==_this14.revertText;
var set=Teams["import"](_this14.textbox.value)[0];
if(!set){
_this14.revertSetToRevertPoint();
if(!_this14.textbox.value.trim()&&!_this14.revertText){
_this14.setState({error:'',copied:false,dirty:false});
}else{
_this14.setState({error:'No Pokemon set found.',copied:false,dirty:dirty});
}
return;
}
...
editor.save();
_this14.props.onChange();
_this14.setState({error:'',copied:false,dirty:dirty});
};return _this14;}_inheritsLoose(SetImportForm,_preact$Component4);var _proto6=SetImportForm.prototype;_proto6.getExportText=function getExportText(){if(!this.props.set)return'';return Teams.exportSet(this.props.set,this.props.editor.dex,true).trim();};_proto6.componentDidMount=function componentDidMount(){this.setRevertPoint();};_proto6.componentDidUpdate=function componentDidUpdate(prevProps){if(prevProps.setIndex===this.props.setIndex)return;this.setRevertPoint();};_proto6.setRevertPoint=function setRevertPoint(){if(!this.textbox)return;this.revertText=this.getExportText();this.refreshText(this.revertText,false);};_proto6.refreshText=function refreshText(){var text=arguments.length>0&&arguments[0]!==undefined?arguments[0]:this.getExportText();var dirty=arguments.length>1&&arguments[1]!==undefined?arguments[1]:text!==this.revertText;if(!this.textbox)return;this.textbox.value=text;this.textbox.focus();this.textbox.select();this.setState({error:'',copied:false,dirty:dirty});};_proto6.revertSetToRevertPoint=function revertSetToRevertPoint(){var _this$props7=this.props,editor=_this$props7.editor,setIndex=_this$props7.setIndex;var set=Teams["import"](this.revertText)[0];if(set){editor.sets[setIndex]=set;}else if(!this.revertText){if(editor.sets[setIndex])editor.sets.splice(setIndex,1);}else{return false;}editor.save();this.props.onChange();return true;};_proto6.
...
StatForm=function(_preact$Component5){function StatForm(){var _this15;for(var _len6=arguments.length,args=new Array(_len6),_key8=0;_key8<_len6;_key8++){args[_key8]=arguments[_key8];}_this15=_preact$Component5.call.apply(_preact$Component5,[this].concat(args))||this;_this15.
...
var _this15$props=_this15.props,editor=_this15$props.editor,set=_this15$props.set;
var team=editor.team;

var guess=new BattleStatGuesser(team.format).guess(set);
set.evs=guess.evs;
_this15.plus=guess.plusStat||null;
_this15.minus=guess.minusStat||null;
_this15.updateNatureFromPlusMinus();
_this15.props.onChange();
};_this15.
handleOptimize=function(){
var _this15$props2=_this15.props,editor=_this15$props2.editor,set=_this15$props2.set;
var team=editor.team;
...
_this15.plus=optimized.plus||null;
_this15.minus=optimized.minus||null;
_this15.updateNatureFromPlusMinus();
_this15.props.onChange();
};_this15.
...
if(_this15.maxEVs()<6*252||_this15.props.editor.isLetsGo){
set.evs||(set.evs={});
}else{
...
var textbox=_this15.base.querySelector("input.stat-input[name=\"ev-"+statID+"\"]");
if(textbox)textbox.value=_this15.getEVText(statID);
}
}
}else{
...
_this15.minus=statID;
}else if(_this15.minus===statID){
_this15.minus=null;
}
_this15.updateNatureFromPlusMinus();
}
...
var unsortedInputs=Array.from(_this15.base.querySelectorAll('.stat-input'));
...
if(modifier==='+'){
_this15.plus=statID;
if(_this15.minus===statID)_this15.minus=null;
}else{
_this15.minus=statID;
if(_this15.plus===statID)_this15.plus=null;
}
_this15.updateNatureFromPlusMinus();
_this15.props.onChange();
};_this15.
...
var set=_this15.props.set;
set.nature=Teams.getNatureFromPlusMinus(_this15.plus,_this15.minus)||undefined;
};_this15.
...
var set=_this15.props.set;
var statID=target.name.split('-')[1];
var value=_this15.dvToIv(target.value);
if(value===null){
if(set.ivs){
...
stats.map(function(_ref7){var _set$evs$statID,_set$evs4;var statID=_ref7[0],statName=_ref7[1],stat=_ref7[2];return preact.h("tr",null,
...
preact.h("td",{"class":"setstatbar"},_this16.renderStatbar(stat,statID)),
...
onInput:_this16.changeEV,onChange:_this16.changeEV,onKeyDown:_this16.keyDownStatInput}
),_this16.renderNatureButtons(statID)),
preact.h("td",null,preact.h("input",{
...
onInput:_this16.changeEV,onChange:_this16.changeEV}
)),
...
"class":"textbox default-placeholder stat-input",onInput:_this16.changeIV,
onChange:_this16.changeIV,onKeyDown:_this16.keyDownStatInput}
)),
...
DetailsForm=function(_preact$Component6){function DetailsForm(){var _this17;for(var _len7=arguments.length,args=new Array(_len7),_key9=0;_key9<_len7;_key9++){args[_key9]=arguments[_key9];}_this17=_preact$Component6.call.apply(_preact$Component6,[this].concat(args))||this;_this17.
...
var target=ev.currentTarget;
var set=_this17.props.set;
if(target.value){
set.name=target.value.trim();
}else{
delete set.name;
}
_this17.props.onChange();
};_this17.
changeTera=function(ev){
var target=ev.currentTarget;
var _this17$props=_this17.props,editor=_this17$props.editor,set=_this17$props.set;
var species=editor.dex.species.get(set.species);
