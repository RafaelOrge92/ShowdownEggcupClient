...
PSTextarea=function(_preact$Component){function PSTextarea(){var _window$CSS;var _this;for(var _len=arguments.length,args=new Array(_len),_key=0;_key<_len;_key++){args[_key]=arguments[_key];}_this=_preact$Component.call.apply(_preact$Component,[this].concat(args))||this;_this.
cssAutosize=!!((_window$CSS=window.CSS)!=null&&_window$CSS.supports!=null&&_window$CSS.supports('field-sizing','content'));_this.
updateSize=function(){
if(_this.cssAutosize)return;
...
pendingTeamScrollRestore=null;_this2.
...
'createfolder':function(name,cmd,elem){var _this3=this;
if(!name){
PS.prompt("Folder name?",{parentElem:elem,okButton:"Create"}).then(function(newName){
newName=(newName||'').trim();
if(!newName)return;

_this3.send("/createfolder "+newName,elem);
});
return;
}
...
select.value=room.curFolder;
var button=_this4.mobileFormatFolderButton;
if(!button)return;
setTimeout(function(){
...
var draggedTeam=_this4.getDraggedTeam(ev);
...
_this4.props.room.updateSearch(target.value);
_this4.forceUpdate();
};_this4.
clearSearch=function(){
var target=_this4.base.querySelector('input[type="search"]');
if(!target)return;
target.value='';
_this4.props.room.updateSearch('');
};_this4.
...
var value=(_querySelector=_this4.base.querySelector('textarea[name="import"]'))==null?void 0:_querySelector.value;
if(!value)return alert('Textarea not found');
if(_this4.props.room.exportMode!==true)return alert('Wrong export mode');
...
preact.h("li",{
key:team.key,onDragEnter:_this6.dragEnterTeam,"data-teamkey":team.key,
...
preact.h(TeamBox,{team:team,onClick:_this6.clearSearch})," ",
